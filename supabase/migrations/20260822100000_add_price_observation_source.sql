...
comment on column public.price_observations.source is
  'Internal provenance for the price observation, e.g. supervalu_direct, dunnes_direct, aldi_direct, pepesto_search.';
...
begin
  ...
  if p_success then
    ...
    insert into public.price_observations (
      ...
      p_store_product_id, p_price, p_was_price, coalesce(p_on_promotion, false), now(), lower(p_store) || '_direct'
    );
...
create or replace function public.finalize_tesco_scrape_product_pepesto(
  p_run_uuid uuid,
  p_store_product_id uuid,
  p_price numeric,
  p_previous_price numeric default null,
  ...
  p_store_product_name text default null,
  p_on_promotion boolean default false,
  p_promo_percentage numeric default 0,
  p_canonical_name text default null
)
returns boolean
language plpgsql
...
begin
  if p_price is null or p_price <= 0 then
    raise exception 'Pepesto finalisation requires positive price';
  end if;

  insert into public.scrape_product_receipts(run_id,store_product_id,outcome)
  values(p_run_uuid,p_store_product_id,'success')
  on conflict(run_id,store_product_id) do nothing;
  ...
  values(p_store_product_id,p_price,null,coalesce(p_on_promotion,false),now(),'pepesto_search');
  ...
  set attempted_count=attempted_count+1,
      ...
      inserted=inserted+case when not v_is_unchanged then 1 else 0 end,
      unchanged_count=unchanged_count+case when v_is_unchanged then 1 else 0 end
  where id=p_run_uuid
