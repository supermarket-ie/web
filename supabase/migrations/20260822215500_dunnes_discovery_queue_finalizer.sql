create table if not exists public.dunnes_discovery_receipts (
  id uuid primary key default gen_random_uuid(),
  run_id uuid not null references public.scrape_runs(id) on delete cascade,
  product_id uuid not null references public.products(id) on delete cascade,
  outcome text not null check (outcome in ('exact','alternative','rejected')),
...
alter table public.dunnes_discovery_receipts enable row level security;
revoke all on public.dunnes_discovery_receipts from anon, authenticated;
grant select, insert, update, delete on public.dunnes_discovery_receipts to service_role;
...
begin
  ...
  if p_outcome = 'exact' then
    ...
    insert into public.price_observations (
      store_product_id, price, observed_at, source
    ) values (
      v_store_product_id, p_price, now(), 'dunnes_direct'
    );
  ...
  where id = p_run_uuid
  returning attempted_count, target_count, extracted
  into v_attempted, v_target, v_extracted;
  ...
  if v_attempted >= coalesce(v_target, 0) and coalesce(v_target, 0) > 0 then
    ...
    from public.dunnes_discovery_receipts
    where run_id = p_run_uuid;
