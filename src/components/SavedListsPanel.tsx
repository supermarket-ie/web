...
export function SavedListsPanel({ token }: { token: string }) {
  ...
  return (
    <div className="mb-5">
      <button
        onClick={() => setOpen(o => !o)}
        className="flex items-center gap-2 text-sm font-semibold text-[#1D2324] hover:text-[#E17055] transition mb-2"
      >
      ...
      {open && (
        <div className="space-y-2">
            ...
            return (
              <div key={list.id} className="bg-white rounded-xl border border-[#E8E2DC] px-4 py-3 flex items-center gap-3">
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium text-[#1D2324] truncate">{list.name}</div>
          ...
          <Link
            href="/"
            className="block text-center text-xs text-[#E17055] font-semibold hover:underline py-1"
          >
            + Build a new list
          </Link>
        </div>
