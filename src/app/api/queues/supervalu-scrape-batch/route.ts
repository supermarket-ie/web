...
export const POST = handleCallback<SupervaluBatchMessage>(
  async (message, metadata) => {
    if (process.env.SUPERVALU_VERCEL_WORKER_ENABLED !== 'true') {
      throw new Error('SUPERVALU_VERCEL_WORKER_ENABLED is not true');
    }
    if (!process.env.SUPABASE_SERVICE_ROLE_KEY) {
      throw new Error('SuperValu queue worker Supabase credential is not configured');
    }
    if (!message?.runUuid || !Array.isArray(message.products) || message.products.length === 0) {
      throw new Error('Invalid SuperValu queue message');
    }
      ...
      try {
        ...
        if (error instanceof TransientSupervaluError) {
          if (metadata.deliveryCount < MAX_TRANSIENT_DELIVERIES) {
            console.warn('[supervalu-queue] transient direct-fetch failure; retrying batch', {
              runId: message.runId,
          ...
          await finalizeSupervaluPermanentFailure(
            message,
            product,
    ...
    console.log('[supervalu-queue] batch complete', {
      runId: message.runId,
      batchIndex: message.batchIndex,
