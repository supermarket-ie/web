...
export async function GET(request: Request) {
  ...
  if (!Number.isInteger(issueNumber) || issueNumber <= 0) return Response.json({ error: 'A valid GitHub issue number is required' }, { status: 400 });

  let issue: GitHubIssue;
  try { issue = await loadIssue(issueNumber); }
  ...
  const operation = OPERATIONS[normalizedTitle];
  const ownerAuthorized = issue.user?.login === OWNER_LOGIN && (!issue.author_association || issue.author_association === 'OWNER');
  if (issue.number !== issueNumber || issue.state !== 'open' || !ownerAuthorized || !operation) {
    return Response.json({
      error: 'GitHub issue is not an authorized production operation',
      checks: {
        number_match: issue.number === issueNumber,
        open: issue.state === 'open',
        owner_authorized: ownerAuthorized,
        operation_allowed: Boolean(operation),
      },
    }, { status: 403 });
  }
  ...
  if (insertError) {
    if (insertError.code === '23505') return Response.json({ operation: operation.name, issue_number: issueNumber, already_dispatched: true });
    return Response.json({ error: insertError.message }, { status: 500 });
  }

  const targetUrl = new URL(operation.target, request.url);
  let status = 'failed'; let targetStatus = 500; let payload: unknown = null;
  try {
    const response = await fetch(targetUrl, { cache: 'no-store', headers: { Authorization: `Bearer ${cronSecret}` } });
    targetStatus = response.status;
    const text = await response.text();
    try { payload = text ? JSON.parse(text) : null; } catch { payload = { body: text.slice(0, 4000) }; }
