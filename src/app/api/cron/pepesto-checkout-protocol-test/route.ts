...
const RUN_ID = 'pepesto_checkout_protocol_20260821';
...
export async function GET(request: Request) {
  ...
  const { data: run, error: runError } = await supabaseAdmin.from('scrape_runs').insert({
    run_id: RUN_ID,
    store: 'tesco',
    retrieval_method: 'pepesto_checkout_protocol',
    started_at: startedAt,
    status: 'running',
    target_count: TEST_PRODUCTS.length,
    threshold_pct: 100,
    attempted_count: 0,
    fetched: 0,
    extracted: 0,
  ...
  try {
    ...
    if (selected.length < TEST_PRODUCTS.length) {
      throw new Error(`Pepesto /products exact URL match only found ${selected.length}/${TEST_PRODUCTS.length}`);
    }

    const skus = selected.map((x) => ({ session_token: x.match.session_token, num_units_to_buy: 1 }));
    if (skus.some((x) => !x.session_token)) throw new Error('Missing one or more Pepesto session tokens');

    const sessionResp = await post('/session', {
      supermarket_domain: 'tesco.ie',
      user_locale: 'en-IE',
      skus,
    });
    ...
    const summary = {
      ...
      charged_cents: {
        products: productsResp.charged,
        session: sessionResp.charged,
        checkout_turn_1: checkoutResp.charged,
        total: productsResp.charged + sessionResp.charged + checkoutResp.charged,
      },
      matched_products: selected.map((x) => ({
        requested_name: x.target.name,
        requested_url: x.target.url,
        returned_name: x.match?.product?.product_name ?? null,
        returned_url: x.match?.product?.product_id ?? null,
      })),
    ...
    await supabaseAdmin.from('scrape_runs').update({
      ...
      target_count: TEST_PRODUCTS.length,
      attempted_count: TEST_PRODUCTS.length,
      fetched: TEST_PRODUCTS.length,
      extracted: TEST_PRODUCTS.length,
      inserted: 0,
      unchanged_count: TEST_PRODUCTS.length,
      failed: 0,
      coverage_pct: 100,
