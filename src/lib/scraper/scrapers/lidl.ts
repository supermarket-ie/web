...
export const PARSE_VERSION = 'lidl:v1';
...
export async function scrape(url: string, ctx: ScrapeContext): Promise<ScrapeResult> {
  ...
  // --- JSON-LD (Lidl uses this reliably) ---
...
function extractSize(html: string): string | undefined {
  // Lidl often has weight/volume in the product description
...
function extractProductName(html: string, text: string): string | null {
  ...
  // Lidl product title h1
  const h1p = html.match(/<h1[^>]*class="[^"]*product[^"]*title[^"]*"[^>]*>([^<]+)<\/h1>/i);
  if (h1p) return h1p[1].trim();
...
function extractPrice(html: string, text: string): number | undefined {
  // Lidl pricebox class
  const m = html.match(/class="[^"]*pricebox[^"]*"[^>]*>\s*[ۣ]?\s*(\d+[.,]?\d*)/i)
...
function extractWasPrice(html: string, text: string): number | undefined {
  const m = html.match(/class="[^"]*(?:was|old|strikethrough)[^"]*"[^>]*>\s*[ۣ]?\s*(\d+[.,]?\d*)/i);
  if (m) return parsePrice(m[1]);
...
function extractPromoText(html: string): string | undefined {
  const m = html.match(/class="[^"]*(?:promo|offer|badge|action|discount)[^"]*"[^>]*>([^<]{3,80})</i);
