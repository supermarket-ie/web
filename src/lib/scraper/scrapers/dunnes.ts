...
export const PARSE_VERSION = 'dunnes:v1';

export async function scrape(url: string, ctx: ScrapeContext): Promise<ScrapeResult> {
  const result = await fetchWithRetry(url, { timeoutMs: ctx.timeoutMs, maxAttempts: 3, delayMinMs: 300, delayMaxMs: 1000 });
...
function extractPrice(html: string, text: string): number | undefined {
  const m = html.match(/class="[^"]*price[^"]*"[^>]*>\s*[ۣ]?\s*(\d+[.,]?\d*)/i);
...
function extractPromoText(html: string): string | undefined {
  const m = html.match(/class="[^"]*(?:promo|offer|badge|discount)[^"]*"[^>]*>([^<]{3,80})</i);
