...
/**
 * Aldi Ireland price extractor.
 *
 * Aldi product pages (groceries.aldi.ie) use structured data
 * and server-rendered price elements.
 */
export const AldiExtractor: StoreExtractor = {
...
function extractPrice(html: string, text: string): number | null {
  // Aldi uses product-price or similar containers
  const priceMatch = html.match(/class="[^"]*product[^"]*price[^"]*"[^>]*>\s*[ۣ]?\s*(\d+\.?\d*)/i);
...
function extractWasPrice(html: string, text: string): number | null {
  const wasMatch = html.match(/class="[^"]*(?:was|old|original)[^"]*"[^>]*>\s*[ۣ]?\s*(\d+\.?\d*)/i);
...
function detectPromotion(html: string, text: string): boolean {
  const promoPatterns = /offer|save|promotion|price\s*drop|reduced|special|super\s*savers/i;
