...
export const householdShopProposalSchema = z.object({
  ...
  items: z.array(z.object({
    ...
    unresolved_need: z.string().trim().min(1).max(200).nullable().optional().describe('Required when canonical_product_id is unknown: describe the requested product for batch matching.'),
    display_label: z.string().trim().min(1).max(200),
    quantity: z.number().int().min(1).max(50).describe('Number of packs or individual units to buy.'),
    unit_or_pack_expectation: z.string().trim().min(1).max(120).describe('Size/content of ONE purchased unit, e.g. 2L bottle, 12-egg pack, or one loose banana. Do not repeat the quantity.'),
