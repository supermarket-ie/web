...
type MealIngredient = { name: string; quantity?: string | null };
type Meal = { day?: string; name?: string; ingredients?: MealIngredient[] };
...
function disliked(name: string, raw?: string | null) {
  if (!raw) return false;
  const n = normalise(name);
  return raw.split(/[,;\n]/).map(normalise).filter(Boolean).some(v => n.includes(v) || v.includes(n));
}
function currentWeekStart(): string {
  const now = new Date(); const day = now.getUTCDay(); const diff = day === 0 ? -6 : 1 - day;
  const monday = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate()));
  monday.setUTCDate(monday.getUTCDate() + diff); return monday.toISOString().slice(0, 10);
}
function stoppedBuying(name: string, household: Household): boolean {
  const memory = household.memory ?? {};
  const candidates = [memory.stoppedItems, memory.stopped_items, memory.droppedItems, memory.dropped_items].flatMap(v => Array.isArray(v) ? v : []);
  return candidates.some(v => normalise(String(v)) === normalise(name));
}
function choosePrice(rows: PriceRow[], preferred?: string[] | null): PriceRow | null {
  const wanted = new Set((preferred ?? []).map(normalise).filter(v => v !== 'all'));
  const preferredRows = wanted.size ? rows.filter(r => wanted.has(normalise(r.store))) : [];
...
export async function buildShopDraft(subscriberId: string) {
  ...
  for (const row of history) { const rows = grouped.get(row.canonical_name) ?? []; rows.push(row); grouped.set(row.canonical_name, rows); }
  ...
  const now = new Date(); const decisions: DraftDecision[] = []; const items: AgentListItem[] = [];

  for (const name of candidateNames) {
    const rows = grouped.get(name) ?? [];
    if (stoppedBuying(name, household) || disliked(name, household.dislikes) || findDietaryViolations([{ canonical_name: name }], household.dietary ?? []).length) {
      decisions.push({ canonical_name: name, action: 'not_added', confidence: 'suppress', reason: 'Not added because an explicit household preference outranks inferred shopping history.', signals: ['explicit household preference'], sources: ['preference'] });
      continue;
    }
    const dates = rows.map(r => new Date(r.observed_at)).sort((a, b) => b.getTime() - a.getTime());
    const intervals = dates.slice(0, -1).map((d, i) => daysBetween(d, dates[i + 1])).filter(v => v >= 1 && v <= 90);
    const interval = median(intervals); const daysSince = dates[0] ? daysBetween(now, dates[0]) : null;
    const frequent = rows.length >= 2; const due = interval != null && daysSince != null && daysSince >= Math.max(1, interval * 0.8);
    const veryRecent = interval != null && daysSince != null && daysSince < interval * 0.45;
    const latestListHas = ((currentList?.items ?? []) as AgentListItem[]).some(i => i.canonical_name === name);
    const include = due || (frequent && interval == null && latestListHas);
    const best = choosePrice(prices.filter(p => p.canonical_name === name), household.preferred_stores);
    if (include && best) {
      const quantity = rows[0]?.quantity ?? ((currentList?.items ?? []) as AgentListItem[]).find(i => i.canonical_name === name)?.quantity ?? 1;
      items.push({ canonical_name: name, category: best.category ?? rows[0]?.category ?? undefined, store: best.store, price: Number(best.price), quantity, on_promotion: Boolean(best.on_promotion) });
      decisions.push({ canonical_name: name, action: 'included', confidence: 'include', reason: interval && daysSince != null ? `Usually bought about every ${Math.round(interval)} days; last bought ${Math.round(daysSince)} days ago.` : 'Frequently bought household item and present in the recent shop pattern.', signals: ['usual product', due ? 'replenishment due' : 'frequent purchase', best.on_promotion ? 'current promotion' : 'current price'], sources: ['history', 'replenishment', 'catalogue'], days_since_last_bought: daysSince == null ? null : Math.round(daysSince), typical_interval_days: interval == null ? null : Math.round(interval), price: Number(best.price), store: best.store, on_promotion: Boolean(best.on_promotion) });
    } else if (veryRecent) {
      decisions.push({ canonical_name: name, action: 'not_added', confidence: 'suppress', reason: `Bought recently${daysSince != null ? ` (${Math.round(daysSince)} days ago)` : ''}; unlikely to be due yet.`, signals: ['recent purchase', 'not replenishment due'], sources: ['history', 'replenishment'], days_since_last_bought: daysSince == null ? null : Math.round(daysSince), typical_interval_days: interval == null ? null : Math.round(interval) });
    }
  }

  const meals = (planData?.meals ?? {}) as Meals;
  const plannedMeals = [...(meals.dinners ?? []), ...(meals.lunches ?? [])];
  const itemKeys = new Set(items.map(i => toEpicureName(i.canonical_name)));
  const mealUsage = new Map<string, { count: number; meals: string[] }>();
  for (const meal of plannedMeals) for (const ingredient of meal.ingredients ?? []) {
    const key = toEpicureName(ingredient.name); if (!key) continue;
    const usage = mealUsage.get(key) ?? { count: 0, meals: [] }; usage.count++;
    const label = [meal.day, meal.name].filter(Boolean).join(' · '); if (label && !usage.meals.includes(label)) usage.meals.push(label); mealUsage.set(key, usage);
  }

  // Meal additions remain suggestions. Precomputed only: draft preparation must stay deterministic and fast.
  const completion = new Map<string, DraftDecision>();
  for (const meal of plannedMeals.slice(0, 6)) {
    const hero = [...new Set((meal.ingredients ?? []).map(i => toEpicureName(i.name)).filter(Boolean))].slice(0, 5);
    if (!hero.length) continue;
    const intel = await getIngredientIntelligence(hero, { dietary: household.dietary, dislikes: household.dislikes, preferred_stores: household.preferred_stores }, 4, { allowLiveFallback: false });
    for (const suggestion of intel.suggestions) {
      const product = suggestion.products[0]; if (!product || itemKeys.has(toEpicureName(product.canonical_name))) continue;
      const existing = completion.get(product.canonical_name);
      const mealLabel = [meal.day, meal.name].filter(Boolean).join(' · ');
      if (existing) { existing.meal_count = (existing.meal_count ?? 1) + 1; existing.signals.push(`useful for ${mealLabel}`); continue; }
      completion.set(product.canonical_name, { canonical_name: product.canonical_name, action: 'suggested', confidence: 'suggest', reason: `${suggestion.ingredient} complements this week's planned meals but needs approval before being added.`, signals: ['meal completion', `useful for ${mealLabel}`, suggestion.signal_count > 1 ? 'ingredient reuse' : 'Epicure pairing'], sources: ['meal_plan', 'ingredient_intelligence', 'catalogue'], meal_count: 1, price: product.price, store: product.store, on_promotion: product.on_promotion });
    }
  }
