...
export default defineTool({
  ...
  async execute(input, ctx) {
    ...
    if (index < 0) {
      return { ok: false, reason: 'item_not_in_shop', message: 'That exact product is not in the current shop.' };
    }
    ...
    return {
      ok: true,
      list_id: shop.id,
      removed: removed.canonical_name,
      remaining_items: shop.items.length,
