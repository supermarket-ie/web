      ...
      ctx.session.auth.current?.principalType === 'user'
        ? defineTool({
            ...
            async execute(input, toolCtx) {
              ...
              const index = shop.items.findIndex(item => item.canonical_product_id === product.canonical_product_id || (!item.canonical_product_id && item.canonical_name === product.canonical_name));
