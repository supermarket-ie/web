      ...
      ctx.session.auth.current?.principalType === 'user'
        ? defineTool({
            ...
            async execute(_input, toolCtx) {
              ...
              return tasks.map(task => ({ id: task.id, type: task.type, canonical_name: task.canonical_name, product_family: task.product_family, condition: task.condition, notification_channel: task.notification_channel, last_triggered_at: task.last_triggered_at, created_at: task.created_at, source_request: task.source_request }));
            },
          })
        : null,
  },
