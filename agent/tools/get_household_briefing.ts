...
type BriefingInsight =
  | { kind: 'promotion' | 'price_drop'; canonical_name: string; store: string; price: number; saving: number; priority: number; meal_context?: MealContext }
  | { kind: 'price_rise'; canonical_name: string; store: string; price: number; increase: number; priority: number; meal_context?: MealContext };
        ...
        ? defineTool({
            ...
            async execute(_input, toolCtx) {
              ...
              const [{ data: history }, { data: weeklyPlan }] = await Promise.all([
                agentSupabase.from('list_items').select('canonical_name, store, price_paid, observed_at').eq('subscriber_id', subscriberId).order('observed_at', { ascending: false }).limit(200),
                agentSupabase.from('weekly_plans').select('meals').eq('subscriber_id', subscriberId).eq('week_start', weekStart).maybeSingle(),
              ]);

              if (!history?.length) return { quiet: true, summary: 'I need a little shopping history before I can give you a useful household briefing.', insights: [] };
              ...
              const [{ data: prices }, { data: pairingRows }] = await Promise.all([
                agentSupabase.from('latest_prices').select('canonical_name, store, price, was_price, on_promotion').in('canonical_name', names),
                agentSupabase.from('product_pairings').select('canonical_name, epicure_key, bridges').eq('found', true).in('canonical_name', names),
              ]);
              ...
              for (const row of (pairingRows ?? []) as PairingRow[]) if (!pairingsByName.has(row.canonical_name)) pairingsByName.set(row.canonical_name, row);
              ...
              function getMealContext(canonicalName: string): { context?: MealContext; bonus: number } {
                ...
                if (epicureKey && mealIngredientMeals.has(epicureKey)) {
                  ...
                  return { bonus: Math.min(18, 10 + relevantMeals.length * 2), context: { relevance: 'planned_ingredient', meals: relevantMeals.slice(0, 4), explanation: `${pairing.epicure_key} is already part of this week's saved meal plan.` } };
                }
                const relatedMeals = new Set<string>();
                for (const bridge of pairing.bridges ?? []) for (const label of mealIngredientMeals.get(toEpicureName(bridge)) ?? []) relatedMeals.add(label);
                if (relatedMeals.size > 0) {
                  return { bonus: Math.min(10, 3 + relatedMeals.size * 2), context: { relevance: 'complements_planned_meal', meals: [...relatedMeals].slice(0, 4), explanation: `${pairing.epicure_key ?? canonicalName} has ingredient-pairing signals relevant to ${relatedMeals.size} planned meal${relatedMeals.size === 1 ? '' : 's'}.` } };
                }
                return { bonus: 0 };
              }
              ...
              for (const name of names) {
                ...
                if (frequent && promoSaving >= 0.75) {
                  insights.push({ kind: 'promotion', canonical_name: name, store: current.store, price: current.price, saving: promoSaving, priority: 80 + promoSaving + mealRelevance.bonus, ...(mealRelevance.context ? { meal_context: mealRelevance.context } : {}) });
                  continue;
                }
                if (change <= -0.5) {
                  insights.push({ kind: 'price_drop', canonical_name: name, store: current.store, price: current.price, saving: Math.abs(change), priority: (frequent ? 60 : 40) + Math.abs(change) + mealRelevance.bonus, ...(mealRelevance.context ? { meal_context: mealRelevance.context } : {}) });
                  continue;
                }
                if (frequent && change >= 1) {
                  insights.push({ kind: 'price_rise', canonical_name: name, store: current.store, price: current.price, increase: change, priority: 35 + change, ...(mealRelevance.context ? { meal_context: mealRelevance.context } : {}) });
                }
              }

              insights.sort((a, b) => b.priority - a.priority);
              const topInsights = insights.slice(0, 3);
              return { quiet: topInsights.length === 0, summary: topInsights.length ? `${topInsights.length} things are worth knowing about this household’s shop.` : 'Nothing important has changed in the household’s usual shop.', insights: topInsights, guidance: 'When meal_context is present, explain why the price or promotion matters to this household rather than presenting it as a generic deal.' };
            },
          })
        : null,
  },
