...
type PlannedIngredient = { name: string; quantity?: string | null };
...
function compactIngredients(meal: PlannedMeal): string[] {
  return [...new Set((meal.ingredients ?? []).map(item => toEpicureName(item.name)).filter(Boolean))].slice(0, 5);
}
    ...
    'turn.started': (_event, ctx) =>
      ctx.session.auth.current?.principalType === 'user'
        ? defineTool({
            ...
            inputSchema: z.object({ kind: z.enum(['dinners', 'lunches', 'all']).default('all'), max_suggestions: z.number().int().min(1).max(10).default(6) }),
            async execute(input, toolCtx) {
              ...
              if (!weeklyPlan?.meals) return { ok: false, reason: 'no_meal_plan', message: 'There is no saved meal plan for this week yet.' };
              if (!shop) return { ok: false, reason: 'no_current_shop', message: 'There is no current shop to compare with the meal plan yet.' };

              const meals = weeklyPlan.meals as WeeklyMeals;
              const selectedMeals: PlannedMeal[] = input.kind === 'dinners' ? (meals.dinners ?? []) : input.kind === 'lunches' ? (meals.lunches ?? []) : [...(meals.dinners ?? []), ...(meals.lunches ?? [])];
              if (selectedMeals.length === 0) return { ok: false, reason: 'no_meals_for_kind', message: `There are no saved ${input.kind === 'all' ? 'meals' : input.kind} to analyse.` };
              ...
              const covered = [...plannedUsage.entries()].filter(([ingredient]) => shopKeys.has(ingredient)).map(([ingredient, usage]) => ({ ingredient, meal_count: usage.count, meals: usage.meals }));
              const plannedButNotExact = [...plannedUsage.entries()].filter(([ingredient]) => !shopKeys.has(ingredient)).map(([ingredient, usage]) => ({ ingredient, meal_count: usage.count, meals: usage.meals })).sort((a, b) => b.meal_count - a.meal_count || a.ingredient.localeCompare(b.ingredient));
              const context = { dietary: (household?.dietary as string[] | null) ?? [], dislikes: (household?.dislikes as string | null) ?? null, preferred_stores: (household?.preferred_stores as string[] | null) ?? [] };
              const completionSignals = new Map<string, { canonical_name: string; ingredient: string; store: string | null; price: number | null; on_promotion: boolean; signal_count: number; meals: string[] }>();
              ...
              for (const meal of selectedMeals.slice(0, 6)) {
                ...
                const intelligence = await getIngredientIntelligence(heroIngredients, context, 4, { allow_live_epicure: false });
                for (const suggestion of intelligence.suggestions) {
                  ...
                  const existing = completionSignals.get(product.canonical_name) ?? { canonical_name: product.canonical_name, ingredient: suggestion.ingredient, store: product.store, price: product.price, on_promotion: product.on_promotion, signal_count: 0, meals: [] };
                  existing.signal_count += Math.max(1, suggestion.signal_count);
                  const label = [meal.day, meal.name].filter(Boolean).join(' · ');
                  if (label && !existing.meals.includes(label)) existing.meals.push(label);
              ...
              const missing_candidates = [...completionSignals.values()]
                ...
                .map(item => ({ ...item, confidence: item.meals.length >= 2 || item.signal_count >= 2 ? 'higher' : 'candidate', reason: item.meals.length >= 2 ? `${item.ingredient} is useful across ${item.meals.length} planned meals and is not currently represented in the shop.` : `${item.ingredient} complements a planned meal and is not currently represented in the shop.` }));
              const reuse_opportunities = [...plannedUsage.entries()].filter(([, usage]) => usage.count > 1).map(([ingredient, usage]) => ({ ingredient, meal_count: usage.count, meals: usage.meals, in_current_shop: shopKeys.has(ingredient) })).sort((a, b) => b.meal_count - a.meal_count || a.ingredient.localeCompare(b.ingredient));
