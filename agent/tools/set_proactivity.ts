    ...
    'turn.started': (_event, ctx) =>
      ctx.session.auth.current?.principalType === 'user'
        ? defineTool({
            ...
            async execute({ mode }, toolCtx) {
              const subscriberId = requireSubscriber(toolCtx);
              const { error } = await agentSupabase.from('subscribers').update({ agent_proactivity: mode, updated_at: new Date().toISOString() }).eq('id', subscriberId);
