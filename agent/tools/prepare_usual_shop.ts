    ...
    'turn.started': (_event, ctx) =>
      ctx.session.auth.current?.principalType === 'user'
        ? defineTool({
            ...
            inputSchema: z.object({ name: z.string().min(1).max(80).optional().describe('Optional name for the prepared draft list.') }),
            async execute(input, toolCtx) {
              const subscriberId = requireSubscriber(toolCtx);
              const draft = await buildShopDraft(subscriberId);
              if (!draft.ok) return { ok: false, reason: draft.reason, message: 'There is not enough household shopping history to prepare a reliable shop yet.' };
              const date = new Date().toLocaleDateString('en-IE', { day: 'numeric', month: 'short' });
              const decisionTrace = { version: 1, prepared_at: new Date().toISOString(), week_start: draft.week_start, decisions: draft.decisions };
              const { data: created, error } = await agentSupabase
                .from('saved_lists')
                .insert({ subscriber_id: subscriberId, name: input.name ?? `Prepared shop · ${date}`, family_size: draft.family_size, meals_prompt: 'Prepared from household shopping patterns, replenishment signals and this week’s meal intent by Supermarket.ie', items: draft.items, store_totals: draft.store_totals, is_default: false, generated_at: new Date().toISOString(), agent_decision_trace: decisionTrace })
                .select('id, name')
                .single();
