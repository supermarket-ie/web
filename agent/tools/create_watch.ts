            ...
            async execute(input, toolCtx) {
              ...
              if (second && second.score >= best.score - 1 && second.canonical_name !== best.canonical_name) {
                return {
                  ...
                  candidates: candidates.slice(0, 3).map(candidate => ({ canonical_name: candidate.canonical_name, best_price: candidate.best_price, best_store: candidate.best_store })),
              ...
              if (input.condition.kind === 'price_below') {
                ...
                if (product?.id) {
                  const { data: existing, error: existingError } = await agentSupabase
                    .from('price_alerts').select('id').eq('subscriber_id', subscriberId).eq('product_id', product.id).eq('active', true).maybeSingle();
              ...
              const task = await createAgentTask({
                subscriberId,
                type: input.condition.kind === 'promotion_started' ? 'promotion_watch' : input.condition.kind === 'available' ? 'availability_watch' : 'price_watch',
              ...
              return {
                ...
                condition: input.condition,
                baseline: snapshot ? { best_price: snapshot.best_price, best_store: snapshot.best_store, on_promotion: snapshot.any_promotion } : null,
