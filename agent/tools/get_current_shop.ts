      ...
      ctx.session.auth.current?.principalType === 'user'
        ? defineTool({
            ...
            async execute(_input, toolCtx) {
              ...
              if (!shop) return { ok: false, reason: 'no_current_shop', message: 'There is no current saved shop yet.' };
              return { ok: true, list_id: shop.id, name: shop.name, generated_at: shop.generated_at, items: shop.items, store_totals: shop.store_totals ?? [], recommended_store: shop.recommended_store ?? null };
            },
          })
        : null,
  },
