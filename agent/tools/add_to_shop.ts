...
export default defineTool({
  ...
  async execute(input, ctx) {
    ...
    if (!current) {
      return { ok: false, reason: 'product_unavailable', message: 'I could not find a current available price for that exact product.' };
    }
    ...
    const { data: created, error: createError } = await agentSupabase
      ...
      .insert({
        ...
        name: 'My agent shop',
        family_size: '2',
        meals_prompt: 'Created by the Supermarket.ie shopping agent',
        items,
        store_totals: totals,
        is_default: false,
