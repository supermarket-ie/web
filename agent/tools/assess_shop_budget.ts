    ...
    'turn.started': (_event, ctx) =>
      ctx.session.auth.current?.principalType === 'user'
        ? defineTool({
            ...
            inputSchema: z.object({ target: z.number().positive().max(2000).optional().describe('Explicit budget target from the user. If omitted, use the stored household weekly budget.') }),
            async execute(input, toolCtx) {
              ...
              const { data: household, error } = await agentSupabase.from('households').select('weekly_budget').eq('subscriber_id', subscriberId).maybeSingle();
              ...
              const highestSpend = shop.items
                .map(item => ({ canonical_name: item.canonical_name, store: item.store ?? null, unit_price: typeof item.price === 'number' ? item.price : null, quantity: item.quantity ?? 1, line_total: Number(((Number(item.price) || 0) * (item.quantity ?? 1)).toFixed(2)) }))
                .sort((a, b) => b.line_total - a.line_total)
                .slice(0, 10);
              if (target == null) return { ok: true, list_id: shop.id, current_total: currentTotal, target: null, highest_spend_items: highestSpend, message: 'No weekly budget is stored for this household. Ask for a target or offer to remember one.' };
