...
type PriceRow = { canonical_name: string; store: string; price: number; on_promotion: boolean | null };
...
function isDisliked(name: string, dislikes?: string | null): boolean {
  ...
  return dislikes.split(/[,;\n]/).map(value => normalise(value)).filter(value => value.length >= 2).some(value => haystack.includes(value) || value.includes(haystack));
}
...
function chooseBestPrice(rows: PriceRow[], preferredStores: string[]): PriceRow | null {
  ...
  return [...(preferredRows.length > 0 ? preferredRows : rows)].sort((a, b) => Number(a.price) - Number(b.price))[0] ?? null;
}
    ...
    'turn.started': (_event, ctx) =>
      ctx.session.auth.current?.principalType === 'user'
        ? defineTool({
            ...
            inputSchema: z.object({ canonical_name: z.string().min(2).describe('Exact canonical product name to find alternatives for.'), limit: z.number().int().min(1).max(8).default(5) }),
            async execute(input, toolCtx) {
              ...
              const [{ data: household, error: householdError }, sourceSnapshot] = await Promise.all([
                agentSupabase.from('households').select('dietary, dislikes, preferred_stores').eq('subscriber_id', subscriberId).maybeSingle(),
              ...
              const { data: targetRows, error: targetError } = await agentSupabase.from('product_pairings').select('canonical_name, epicure_key, bridges').eq('canonical_name', input.canonical_name).eq('found', true).limit(1);
              ...
              const candidatesByName = new Map<string, { canonical_name: string; ingredient: string | null; similarity_signals: number; source: 'precomputed' | 'live_epicure' }>();

              if (target?.bridges?.length) {
                const targetBridges = new Set(target.bridges);
                const { data: candidates, error: candidateError } = await agentSupabase.from('product_pairings').select('canonical_name, epicure_key, bridges').eq('found', true).neq('canonical_name', input.canonical_name).limit(300);
                ...
                for (const candidate of (candidates ?? []) as PairingRow[]) {
                  ...
                  if (!existing || overlap > existing.similarity_signals) candidatesByName.set(candidate.canonical_name, { canonical_name: candidate.canonical_name, ingredient: candidate.epicure_key, similarity_signals: overlap, source: 'precomputed' });
                }
              }
              ...
              if (candidatesByName.size < input.limit) {
                ...
                for (const substitute of live?.substitutes ?? []) {
                  const grounded = await groundIngredient(substitute.name, { dietary, dislikes, preferred_stores: preferredStores }, 2);
                  for (const product of grounded) {
                    ...
                    candidatesByName.set(product.canonical_name, { canonical_name: product.canonical_name, ingredient: substitute.name, similarity_signals: Math.max(1, Math.round(substitute.similarity * 10)), source: 'live_epicure' });
                  }
                  if (candidatesByName.size >= Math.max(input.limit * 2, 10)) break;
                }
              }

              const ranked = [...candidatesByName.values()].sort((a, b) => b.similarity_signals - a.similarity_signals).slice(0, 30);
              if (ranked.length === 0) return { ok: false, reason: 'no_substitutes', message: 'I could not find a sufficiently grounded substitute that fits this household.' };
              const names = ranked.map(row => row.canonical_name);
              const { data: priceRows, error: pricesError } = await agentSupabase.from('latest_prices').select('canonical_name, store, price, on_promotion').in('canonical_name', names);
              ...
              const substitutes = ranked
                .map(candidate => {
                  const best = chooseBestPrice(prices.filter(row => row.canonical_name === candidate.canonical_name), preferredStores);
                  if (!best) return null;
                  const price = Number(best.price);
                  return { canonical_name: candidate.canonical_name, ingredient_role: candidate.ingredient, similarity_signals: candidate.similarity_signals, intelligence_source: candidate.source, store: best.store, price, on_promotion: Boolean(best.on_promotion), saving_vs_source_best: sourcePrice == null ? null : Number((sourcePrice - price).toFixed(2)) };
                })
                .filter((row): row is NonNullable<typeof row> => row !== null)
                .sort((a, b) => a.similarity_signals !== b.similarity_signals ? b.similarity_signals - a.similarity_signals : a.price - b.price)
                .slice(0, input.limit);
              ...
              return {
                ...
                household_constraints_applied: { dietary, dislikes: dislikes ?? null, preferred_stores: preferredStores },
                substitutes,
