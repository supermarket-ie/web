...
export default defineTool({
  ...
  async execute(input, ctx) {
    ...
    if (Object.keys(fields).length === 0) {
      return { ok: false, reason: 'no_changes', message: 'No household preference changes were provided.' };
    }

    const { error } = await agentSupabase
      .from('households')
      .upsert({ subscriber_id: subscriberId, ...fields }, { onConflict: 'subscriber_id' });
