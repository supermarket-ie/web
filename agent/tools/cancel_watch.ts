    ...
    'turn.started': (_event, ctx) =>
      ctx.session.auth.current?.principalType === 'user'
        ? defineTool({
            ...
            async execute({ taskId }, toolCtx) {
              const subscriberId = requireSubscriber(toolCtx);
              const { data: task, error: taskError } = await agentSupabase.from('agent_tasks').select('baseline').eq('id', taskId).eq('subscriber_id', subscriberId).single();
              if (taskError) throw new Error(`Could not find watch: ${taskError.message}`);
              const legacyAlertId = task?.baseline && typeof task.baseline === 'object' ? (task.baseline as Record<string, unknown>).legacy_alert_id : null;
              if (typeof legacyAlertId === 'string') {
                const { error: alertError } = await agentSupabase.from('price_alerts').update({ active: false }).eq('id', legacyAlertId).eq('subscriber_id', subscriberId);
